...
async function openStream({text,mood,voiceStyle,key}){
  ...
  try{
    const upstream=await fetch(INTERACTIONS_URL,{
      ...
      body:JSON.stringify({model:TTS_MODEL,input:buildTtsInput(text,mood,voiceStyle),response_format:{type:"audio"},generation_config:{speech_config:[{voice:TTS_VOICE}]},stream:true})
    });
    clearTimeout(timer);
    if(upstream.ok&&upstream.body)return upstream;
    let details={};try{details=await upstream.json()}catch(_){details={}}
    ...
    clearTimeout(timer);
    if(err?.name==="AbortError"){const e=new Error("Gemini TTS stream timeout");e.code="ETIMEDOUT";e.status=504;throw e}
    throw err;
  }
}
