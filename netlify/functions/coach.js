...
exports.handler=async(event)=>{
  ...
  try{
    if(event.httpMethod==="GET"&&params.probe==="1"){
      if(!process.env.GEMINI_API_KEY)return {statusCode:500,headers,body:JSON.stringify({ok:false,provider:"gemini",model:PRIMARY_MODEL,keyConfigured:false,error:"GEMINI_API_KEY is not configured",ttsConfigured:Boolean(process.env.ELEVENLABS_API_KEY&&TTS_VOICE_ID),ttsProvider:TTS_PROVIDER,ttsModel:TTS_MODEL,build:"39.8"})};
      return {statusCode:200,headers,body:JSON.stringify({ok:true,provider:"gemini",model:PRIMARY_MODEL,keyConfigured:true,message:"STACK ZERO function ready",ttsConfigured:Boolean(process.env.ELEVENLABS_API_KEY&&TTS_VOICE_ID),ttsProvider:TTS_PROVIDER,ttsModel:TTS_MODEL,build:"39.8"})};
    }
    const body=event.body?JSON.parse(event.body):{};
    if(body.mode==="tts"){
      const r=await generateTTS(body.text,body.mood,body.voiceStyle);
      return {statusCode:200,headers,body:JSON.stringify({ok:true,keyConfigured:true,build:"39.8",...r})};
    }
    if(!process.env.GEMINI_API_KEY)return {statusCode:500,headers,body:JSON.stringify({ok:false,provider:"gemini",model:PRIMARY_MODEL,keyConfigured:false,error:"GEMINI_API_KEY is not configured",build:"39.8"})};
    const r=await generateAssistant(body);
    return {statusCode:200,headers,body:JSON.stringify({ok:true,provider:"gemini",keyConfigured:true,ttsConfigured:Boolean(process.env.ELEVENLABS_API_KEY&&TTS_VOICE_ID),ttsProvider:TTS_PROVIDER,ttsModel:TTS_MODEL,ttsVoiceId:TTS_VOICE_ID||null,build:"39.8",...r})};
  }catch(err){
    let parsed={};try{parsed=event.body?JSON.parse(event.body):{}}catch(_){}
    const isTts=parsed?.mode==="tts";
    console.error("[STACK ZERO 39.7 coach]",{provider:isTts?TTS_PROVIDER:"gemini",mode:isTts?"tts":event.httpMethod,status:err?.status||null,code:err?.code||null,message:err?.message||String(err)});
