...
exports.handler=async(event)=>{
  ...
  try{
    ...
    if(body.mode==="tts"){const r=await generateTTS(body.text,body.mood);return {statusCode:200,headers,body:JSON.stringify({ok:true,provider:"gemini-tts",keyConfigured:true,...r})}}
  ...
  }catch(err){const status=err.status&&err.status>=400&&err.status<600?err.status:(err.code==="ETIMEDOUT"?504:502);return {statusCode:status,headers,body:JSON.stringify({ok:false,provider:"gemini",model:PRIMARY_MODEL,ttsModel:TTS_MODEL,keyConfigured,error:err.message,details:err.details||undefined})}}
};
