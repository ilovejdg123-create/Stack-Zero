...
module.exports=async function handler(req,res){
  ...
  if(req.method!=="POST"&&!isProbe)return res.status(405).json({ok:false,error:"Method not allowed",build:"40.0"});
  let body={};
  try{
    if(isProbe){
      if(!process.env.GEMINI_API_KEY)return res.status(500).json({ok:false,provider:"gemini",model:PRIMARY_MODEL,keyConfigured:false,error:"GEMINI_API_KEY is not configured",ttsConfigured:Boolean(process.env.ELEVENLABS_API_KEY&&TTS_VOICE_ID),ttsProvider:TTS_PROVIDER,ttsModel:TTS_MODEL,build:"40.0",platform:"vercel"});
      return res.status(200).json({ok:true,provider:"gemini",model:PRIMARY_MODEL,keyConfigured:true,message:"STACK ZERO Vercel function ready",ttsConfigured:Boolean(process.env.ELEVENLABS_API_KEY&&TTS_VOICE_ID),ttsProvider:TTS_PROVIDER,ttsModel:TTS_MODEL,build:"40.0",platform:"vercel"});
    }
    body=typeof req.body==="string"?JSON.parse(req.body||"{}"):req.body||{};
    if(body.mode==="tts"){
      const r=await generateTTS(body.text,body.mood,body.voiceStyle);
      return res.status(200).json({ok:true,keyConfigured:true,build:"40.0",platform:"vercel",...r});
    }
    if(!process.env.GEMINI_API_KEY)return res.status(500).json({ok:false,provider:"gemini",model:PRIMARY_MODEL,keyConfigured:false,error:"GEMINI_API_KEY is not configured",build:"40.0",platform:"vercel"});
    const r=await generateAssistant(body);
    return res.status(200).json({ok:true,provider:"gemini",keyConfigured:true,ttsConfigured:Boolean(process.env.ELEVENLABS_API_KEY&&TTS_VOICE_ID),ttsProvider:TTS_PROVIDER,ttsModel:TTS_MODEL,ttsVoiceId:TTS_VOICE_ID||null,build:"40.0",platform:"vercel",...r});
  }catch(err){
    const isTts=body?.mode==="tts";
    console.error("[STACK ZERO 40.0 Vercel coach]",{provider:isTts?TTS_PROVIDER:"gemini",mode:isTts?"tts":req.method,status:err?.status||null,code:err?.code||null,message:err?.message||String(err)});
    const status=err?.status&&err.status>=400&&err.status<600?err.status:(err?.code==="ETIMEDOUT"||err?.code==="TIMEOUT"?504:502);
    return res.status(status).json({ok:false,provider:isTts?TTS_PROVIDER:"gemini",model:isTts?TTS_MODEL:PRIMARY_MODEL,keyConfigured:isTts?Boolean(process.env.ELEVENLABS_API_KEY):Boolean(process.env.GEMINI_API_KEY),error:err?.message||String(err),code:err?.code||null,details:err?.details||undefined,retryAfterMs:Number(err?.retryAfterMs||err?.details?.retryAfterMs||0)||0,build:"40.0",platform:"vercel"});
  }
};
