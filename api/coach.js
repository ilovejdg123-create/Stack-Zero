...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    // ?probe=1 is a REAL Gemini request, not just a key/status check
    if (req.query?.probe === '1') {
      if (!key) return res.status(500).json({ ok:false, provider:'gemini', error:'GEMINI_API_KEY is not configured', code:'MISSING_KEY' });
      ...
      try {
        const r = await fetch(url, {
          ...
          body: JSON.stringify({
            contents: [{ parts: [{ text: 'Reply with exactly GEMINI_OK and nothing else.' }] }]
          })
        });
        ...
        if (!r.ok) {
          return res.status(200).json({
            ...
            geminiHttpStatus:r.status,
            code:data?.error?.status || `HTTP_${r.status}`,
            error:data?.error?.message || text || 'Gemini returned an error'
          });
        }
        const reply = data?.candidates?.[0]?.content?.parts?.map(p=>p.text||'').join('').trim() || '';
        return res.status(200).json({ ok:true, provider:'gemini', model, reply, tested:'real_gemini_generateContent' });
      } catch (e) {
        return res.status(200).json({
          ok:false, provider:'gemini', model,
