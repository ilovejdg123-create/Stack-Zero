...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") return res.status(200).json({ ok:true, service:"STACK ZERO Kaguya Coach", timestamp:new Date().toISOString() });
...
같은 숫자라도 같은 칭찬 문구를 반복하지 마라. 이전 응답과 같은 첫 문장 시작, 같은 감탄사, 같은 호칭 패턴도 피하라. 최근 응답을 참고하되 그대로 인용하거나 재배열하지 마라.`;
