...
const DEFAULT_VOICE=process.env.AZURE_SPEECH_VOICE||"ja-JP-MayuNeural";
...
function prosody(mood){
  ...
  // Baseline is a little faster/higher; Mayu gets a subtle extra pitch lift in V40.6.
  const map={
  ...
  const base=map[m]||map.normal;
  return base
}
function addPercent(value,delta){const n=parseFloat(String(value||"0").replace("%",""))||0;return `${n+delta>=0?"+":""}${n+delta}%`}
...
module.exports=async function handler(req,res){
  ...
  if(req.method!=="POST")return res.status(405).json({ok:false,error:"Method not allowed",build:"40.6",platform:"vercel"});
  const key=process.env.AZURE_SPEECH_KEY,region=String(process.env.AZURE_SPEECH_REGION||"").trim();
  if(!key||!region)return res.status(500).json({ok:false,error:"AZURE_SPEECH_KEY or AZURE_SPEECH_REGION is not configured",code:"AZURE_TTS_NOT_CONFIGURED",build:"40.6",platform:"vercel"});
  try{
    const body=typeof req.body==="string"?JSON.parse(req.body||"{}"):req.body||{};
    const text=String(body.text||"").trim().slice(0,700);
    if(!text)return res.status(400).json({ok:false,error:"text is required",build:"40.6",platform:"vercel"});
    ...
    const p=prosody(body.mood),pitch=voice==="ja-JP-MayuNeural"?addPercent(p.pitch,2):p.pitch;
    const spoken=`<prosody rate="${p.rate}" pitch="${pitch}">${escapeXml(text)}</prosody>`;
    ...
    return res.status(200).json({ok:true,provider:"azure-speech",model:"Azure Japanese Neural TTS",voice,audioBase64:bytes.toString("base64"),mimeType:r.headers.get("content-type")||"audio/mpeg",build:"40.6",platform:"vercel"});
  }catch(err){
    if(err?.name==="AbortError"){err=new Error("Azure TTS timeout");err.status=504;err.code="TIMEOUT"}
    const status=err?.status&&err.status>=400&&err.status<600?err.status:502;
    return res.status(status).json({ok:false,provider:"azure-speech",error:err?.message||String(err),code:err?.code||null,retryAfterMs:Number(err?.retryAfterMs||0)||0,build:"40.6",platform:"vercel"});
  }
};
