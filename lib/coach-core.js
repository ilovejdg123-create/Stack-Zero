...
- study_done은 "스택 버튼을 눌렀다"가 아니라 "방금 공부 한 시간을 더 끝냈다"는 현실 사건이다. 오늘 study=7이면 누적 7시간 공부한 상태다
- study_done 대사에서 사용자를 향해 "7개", "7 STACK", "스택 7개"라고 말하는 것은 금지한다. 숫자를 언급할 필요가 있을 때만 "7시간", "일곱 시간째"처럼 현실 시간으로 말한다
- study_done에서 정확한 시간 숫자를 매번 말하지 않는다. 오히려 대부분은 집중력, 시간의 흐름, 지금까지 오래 앉아 있던 모습, 피곤함을 감추는 태도, 계속 해내는 모습 등에 사람처럼 반응한다
- 공부 1~2시간은 과한 칭찬을 피하고 시작/페이스 정도로 본다. 3~4시간은 슬슬 오래 붙잡고 있음을 눈치챈다. 5~6시간은 꽤 했다는 인정이나 가벼운 놀림이 자연스럽다. 7시간부터는 정말 오래 공부한 날이라는 놀람과 인정이 조금 새어도 된다. 10시간 이상은 평소보다 감정이 더 크게 새지만 여전히 과장된 코치 멘트는 금지한다
...
async function generateAssistant(body){
  ...
  try{const r=await callGeminiRaw(prompt,PRIMARY_MODEL,6500,true,0);return {...parseAssistant(r.raw),model:r.model}}
...
function ttsDirection(mood,voiceStyle=""){
  const map={
    ...
    normal:"refined, intelligent and youthful; slightly brighter and a little higher in register than neutral, with clearly expressive Japanese pitch contour, varied cadence, subtle smiles and restrained warmth"
  };
...
function buildTtsInput(text,mood,voiceStyle=""){
  return `Synthesize ONLY the Japanese transcript as natural speech.\nAudio profile: a refined, intelligent Japanese woman in a calm low-mid register. The voice should feel poised, elegant and youthful-adult, never cute, squeaky, overly bright, or announcer-like.\nScene: private casual conversation with someone she knows well.\nDirector's notes: ${ttsDirection(mood,voiceStyle)}. Use restrained but expressive Japanese pitch movement: controlled and elegant most of the time, with emotion leaking through briefly when surprised, flustered, pleased, worried, or teasing. Keep the pitch lower and steadier than a typical youthful TTS voice. Use natural pauses, small breath changes, and conversational sentence endings. Do not sound flat, robotic, announcer-like, sugary-cute, or exaggerated. Do not read any instructions aloud.\nSpoken transcript begins after this line:\n${String(text||"").trim().slice(0,500)}`;
}
...
async function generateTTSViaGenerateContent(text,mood,voiceStyle,key){
  const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),10500);
  try{
    const url=`${API_BASE}${encodeURIComponent(TTS_MODEL)}:generateContent?key=${encodeURIComponent(key)}`;
    const r=await fetch(url,{method:"POST",headers:{"Content-Type":"application/json"},signal:controller.signal,body:JSON.stringify({
    ...
    const data=await r.json().catch(()=>({}));
    if(!r.ok){const e=new Error(data?.error?.message||`Gemini TTS GenerateContent HTTP ${r.status}`);e.status=r.status;e.details=data?.error||data;throw e}
    const audio=data?.candidates?.[0]?.content?.parts?.find(p=>p?.inlineData?.data)?.inlineData?.data||findAudioData(data);
    if(!audio)throw new Error("Gemini GenerateContent TTS returned no audio");
    return {audioBase64:audio,model:TTS_MODEL,voice:TTS_VOICE,sampleRate:24000,transport:"generateContent"};
  }finally{clearTimeout(timer)}
}
async function generateTTS(text,mood,voiceStyle=""){
  ...
  for(let attempt=0;attempt<2;attempt++){
    ...
    try{
      const r=await fetch(INTERACTIONS_URL,{method:"POST",headers:{"Content-Type":"application/json","x-goog-api-key":key,"Api-Revision":"2026-05-20"},signal:controller.signal,body:JSON.stringify({model:TTS_MODEL,input:buildTtsInput(text,mood,voiceStyle),response_format:{type:"audio"},generation_config:{speech_config:[{voice:TTS_VOICE}]}})});
      const data=await r.json().catch(()=>({}));if(!r.ok){const e=new Error(data?.error?.message||`Gemini TTS HTTP ${r.status}`);e.status=r.status;e.details=data?.error||data;throw e}
      const audio=findAudioData(data);if(!audio)throw new Error("Gemini TTS returned no audio");return {audioBase64:audio,model:TTS_MODEL,voice:TTS_VOICE,sampleRate:24000,transport:"interactions"};
    }catch(err){
      if(err?.name==="AbortError"){const e=new Error("Gemini TTS timeout");e.code="ETIMEDOUT";lastErr=e}else lastErr=err;
      const transient=lastErr?.code==="ETIMEDOUT"||retryableStatus(Number(lastErr?.status||0));
      if(!transient||attempt>=1)break;
      await sleep(Math.round((220*Math.pow(2,attempt))+(Math.random()*160)));
    }finally{clearTimeout(timer)}
  }
  try{return await generateTTSViaGenerateContent(text,mood,voiceStyle,key)}catch(err){throw lastErr||err}
}

module.exports={PRIMARY_MODEL,FAST_MODEL,TTS_MODEL,TTS_VOICE,generateAssistant,generateTTS,callGeminiRaw,buildTtsInput,retryableStatus};
