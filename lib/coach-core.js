...
반드시 JSON 하나만 출력한다. 마크다운 금지.
{
  ...
  "voiceStyle": "짧은 영어 음성 연기 지시문. 감정, 숨, 속도, 억양을 자연스럽게 1문장으로. 대사 내용은 넣지 않음",
...
function ttsDirection(mood,voiceStyle=""){
  const map={
    ...
    happy:"genuinely delighted and unable to fully hide it; brighter smile in the voice, quickened tempo, sparkly sentence endings",
    flustered:"embarrassed and flustered while trying to stay composed; breath catches, softer volume, shy hesitations and evasive cadence",
    smug:"playfully smug and teasing; elegant confidence, light laugh in the breath, slightly slower emphasis on punch words",
    surprised:"sudden genuine surprise with widened emotional pitch; immediate reaction first, then quick recovery into composure",
...
async function generateTTS(text,mood,voiceStyle=""){
  ...
  for(let attempt=0;attempt<3;attempt++){
    const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),11500);
    try{
      ...
      const audio=findAudioData(data);if(!audio)throw new Error("Gemini TTS returned no audio");return {audioBase64:audio,model:TTS_MODEL,voice:TTS_VOICE,sampleRate:24000};
    }catch(err){
      ...
      await sleep(Math.round((250*Math.pow(2,attempt))+(Math.random()*180)));
    }finally{clearTimeout(timer)}
  }
