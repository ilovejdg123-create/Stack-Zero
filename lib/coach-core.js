...
function normalizeMood(v){const m=String(v||"").toLowerCase();return ["angry","sulk","question","cry","normal"].includes(m)?m:"normal"}
...
오늘: 공부 ${today.study||0} STACK / 운동 ${today.exercise||0} / 잠 ${today.sleep||0}
...
반드시 JSON 하나만 출력한다. 마크다운 금지.
{
  ...
  "mood": "normal|angry|sulk|question|cry 중 하나",
...
async function callGeminiRaw(prompt,model,timeoutMs,jsonMode=true){
  ...
  try{
    ...
    const generationConfig={maxOutputTokens:420,temperature:1.12,topP:.96};if(jsonMode)generationConfig.responseMimeType="application/json";
    ...
    const data=await r.json().catch(()=>({}));if(!r.ok){const e=new Error(data?.error?.message||`Gemini HTTP ${r.status}`);e.status=r.status;e.details=data?.error||data;throw e}
    const raw=textFromGemini(data);if(!raw)throw new Error("Gemini returned no text");return {raw,model};
  }catch(err){if(err?.name==="AbortError"){const e=new Error(`Gemini timeout after ${timeoutMs}ms`);e.code="ETIMEDOUT";throw e}throw err}finally{clearTimeout(timer)}
}
function canFallback(err){return err?.code==="ETIMEDOUT"||err?.status===404||err?.status===429||(err?.status>=500&&err?.status<=599)}
async function generateAssistant(body){const prompt=promptFor(body);try{const r=await callGeminiRaw(prompt,PRIMARY_MODEL,6500,true);return {...parseAssistant(r.raw),model:r.model}}catch(err){if(FAST_MODEL!==PRIMARY_MODEL&&canFallback(err)){const r=await callGeminiRaw(prompt,FAST_MODEL,4000,true);return {...parseAssistant(r.raw),model:r.model}}throw err}}

function ttsDirection(mood){const map={angry:"controlled irritation, elegant and restrained, slightly sharper consonants",sulk:"slightly sulky and reserved, soft but clearly displeased",question:"curious and intelligent, lightly teasing, natural rising nuance",cry:"soft, emotionally shaken but trying to remain composed",normal:"elegant, intelligent, youthful young-adult Japanese woman; restrained warmth, subtle teasing, natural conversational pacing"};return map[normalizeMood(mood)]||map.normal}
...
async function generateTTS(text,mood){const key=process.env.GEMINI_API_KEY;if(!key)throw new Error("GEMINI_API_KEY is not configured");const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),10500);try{const input=`Audio profile: a refined, intelligent, youthful Japanese woman. Not childlike, not exaggerated anime acting.\nScene: private casual conversation with someone she knows well.\nDirector's notes: ${ttsDirection(mood)}. Speak natural Japanese only. Do not read these instructions aloud.\nTranscript:\n${String(text||"").trim().slice(0,500)}`;const r=await fetch(INTERACTIONS_URL,{method:"POST",headers:{"Content-Type":"application/json","x-goog-api-key":key,"Api-Revision":"2026-05-20"},signal:controller.signal,body:JSON.stringify({model:TTS_MODEL,input,response_format:{type:"audio"},generation_config:{speech_config:[{voice:TTS_VOICE}]}})});const data=await r.json().catch(()=>({}));if(!r.ok){const e=new Error(data?.error?.message||`Gemini TTS HTTP ${r.status}`);e.status=r.status;e.details=data?.error||data;throw e}const audio=findAudioData(data);if(!audio)throw new Error("Gemini TTS returned no audio");return {audioBase64:audio,model:TTS_MODEL,voice:TTS_VOICE,sampleRate:24000}}catch(err){if(err?.name==="AbortError"){const e=new Error("Gemini TTS timeout");e.code="ETIMEDOUT";throw e}throw err}finally{clearTimeout(timer)}}

module.exports={PRIMARY_MODEL,FAST_MODEL,TTS_MODEL,generateAssistant,generateTTS,callGeminiRaw};
