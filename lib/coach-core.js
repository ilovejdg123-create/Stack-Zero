...
async function generateTTS(text,mood,voiceStyle=""){
  const key=process.env.GEMINI_API_KEY;if(!key)throw new Error("GEMINI_API_KEY is not configured");
  // Reliability first: current official GenerateContent TTS path is the primary transport.
  // This avoids waiting on a failing streaming/Interactions request before the usable fallback.
  let firstErr=null;
  for(let attempt=0;attempt<2;attempt++){
    try{return await generateTTSViaGenerateContent(text,mood,voiceStyle,key)}
    catch(err){firstErr=err;const transient=err?.name==="AbortError"||err?.code==="ETIMEDOUT"||retryableStatus(Number(err?.status||0));if(!transient||attempt>=1)break;await sleep(260+Math.round(Math.random()*180))}
  }
  // Last-resort official Interactions path, one short attempt only.
  const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),6500);
  try{
    const r=await fetch(INTERACTIONS_URL,{method:"POST",headers:{"Content-Type":"application/json","x-goog-api-key":key},signal:controller.signal,body:JSON.stringify({model:TTS_MODEL,input:buildTtsInput(text,mood,voiceStyle),response_format:{type:"audio"},generation_config:{speech_config:[{voice:TTS_VOICE}]}})});
    ...
    const audio=findAudioData(data);if(!audio)throw new Error("Gemini TTS returned no audio");return {audioBase64:audio,model:TTS_MODEL,voice:TTS_VOICE,sampleRate:24000,transport:"interactions-fallback"};
  }catch(err){throw firstErr||err}finally{clearTimeout(timer)}
}
