...
반드시 JSON 하나만 출력한다. 마크다운 금지.
{
  ...
  "voiceStyle": "짧은 영어 음성 연기 지시문. 감정, 숨, 속도, 억양을 자연스럽게 1문장으로. 대사 내용은 넣지 않음. 기본은 차분하고 품위 있지만 감정이 새는 순간에는 밝은 상중음역의 작은 피치 상승과 텐션 변화를 허용한다. childish, squeaky, bubbly는 금지",
...
function ttsDirection(mood,voiceStyle=""){
  const map={
    ...
    normal:"refined, intelligent and youthful; calm but naturally bright upper-mid register, clear delicate tone, poised Japanese pitch contour and expressive timing; restrained at baseline, with small lively pitch lifts when emotion leaks through; never childish, squeaky or bubbly"
  };
...
function buildTtsInput(text,mood,voiceStyle=""){
  ...
  const extra=String(voiceStyle||"").trim();
  return `Speak this Japanese line naturally and only speak the transcript. Voice: a poised youthful Japanese woman with a clear, naturally bright upper-mid register. Her baseline is calm, elegant and controlled, but never flat: let emotion briefly leak through with subtle pitch lifts, sharper timing, tiny caught breaths or a smile when the scene calls for it. Slightly higher than a neutral adult female voice, but never childish, squeaky or bubbly. Use natural Japanese pitch accent and intimate conversational rhythm rather than narrator delivery. ${moodTag} ${extra?"Acting note: "+extra+". ":""}Transcript: ${String(text||"").trim().slice(0,420)}`;
}
