...
function buildTtsInput(text,mood,voiceStyle=""){
  ...
  const extra=String(voiceStyle||"").trim();
  return `Speak this Japanese line naturally and only speak the transcript. Voice: a poised youthful Japanese woman with a warm base and a slightly lighter, thinner upper-mid timbre. Keep the calm elegance of the warm voice, but lift the perceived pitch only a little—subtle, not dramatically high. Use clear natural Japanese pitch accent, intimate conversational rhythm, and let emotion leak through with small pitch lifts, tiny caught breaths, soft laughter or a smile when appropriate. Never childish, squeaky, shrill or bubbly. ${moodTag} ${extra?"Acting note: "+extra+". ":""}Transcript: ${String(text||"").trim().slice(0,420)}`;
}
...
async function generateTTSViaGenerateContent(text,mood,voiceStyle,key,model){
  ...
  try{
    ...
    const data=await r.json().catch(()=>({}));
    if(!r.ok){const message=data?.error?.message||`Gemini TTS HTTP ${r.status}`;const e=new Error(message);e.status=r.status;e.code=data?.error?.status||"";e.details=data?.error||data;e.model=model;e.retryAfterMs=parseRetryAfterMs(r.headers?.get?.("retry-after"),message);throw e}
...
async function generateTTS(text,mood,voiceStyle=""){
  const key=process.env.GEMINI_API_KEY;if(!key)throw new Error("GEMINI_API_KEY is not configured");
  const models=[TTS_MODEL,TTS_FALLBACK_MODEL].filter((v,i,a)=>v&&a.indexOf(v)===i);
  const attempts=[];
  for(const model of models){
    try{return await generateTTSViaGenerateContent(text,mood,voiceStyle,key,model)}
    catch(err){
      attempts.push({model,status:Number(err?.status||0)||null,code:String(err?.code||""),message:String(err?.message||err).slice(0,220),retryAfterMs:Number(err?.retryAfterMs||0)||0});
      const status=Number(err?.status||0),fallbackAllowed=status===404||status===408||status===429||status>=500||err?.code==="TIMEOUT"||err?.code==="NO_AUDIO";
      if(!fallbackAllowed)break;
    }
  }
  const last=attempts[attempts.length-1]||{};
  const allRateLimited=attempts.length===models.length&&attempts.every(a=>a.status===429);
  const e=new Error(allRateLimited?"Gemini TTS rate limit reached":(last.message||"Gemini TTS failed"));
  e.status=allRateLimited?429:(last.status||502);e.code=allRateLimited?"TTS_RATE_LIMITED":(last.code||"TTS_FAILED");
  const suggested=Math.max(...attempts.map(a=>Number(a.retryAfterMs||0)),0);
  e.retryAfterMs=allRateLimited?Math.max(60000,suggested||600000):suggested;
  e.details={attempts,retryAfterMs:e.retryAfterMs,rateLimited:allRateLimited};throw e;
}
